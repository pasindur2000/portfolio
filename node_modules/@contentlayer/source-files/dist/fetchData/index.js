...
export const fetchData = ({ coreSchemaDef, documentTypeDefs, flags, options, contentDirPath, verbose, }) => {
    ...
    const fileUpdatesStream = (S.mapEitherRight(chokidarAllEventToCustomUpdateEvent)(FSWatch.makeAndSubscribe('.', {
        cwd: contentDirPath,
        ignoreInitial: true,
        // Unfortunately needed in order to avoid race conditions
        awaitWriteFinish: { stabilityThreshold: 50, pollInterval: 10 },
    })));
    const resolveParams = (T.either(core.DataCache.loadPreviousCacheFromDisk({ schemaHash: coreSchemaDef.hash }), fileName_1 + ":47:116"));
    ...
    S.tapRight((cache_) => T.succeedWith(() => (cache = cache_), fileName_1 + ":97:45"))(S.mapEffectEitherRight((event) => (T.either(T.matchTag({
    ...
    })(event), fileName_1 + ":93:21")))(S.startWithRight(initEvent)(S.tapRight((e) => T.succeedWith(() => (e._tag === 'updated' || e._tag === 'deleted') && console.log(`\nFile ${e._tag}: ${e.relativeFilePath}`), fileName_1 + ":55:24"))(fileUpdatesStream)))))))(S.fromEffect(resolveParams)))));
};
