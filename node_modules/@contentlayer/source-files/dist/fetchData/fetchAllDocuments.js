...
export const fetchAllDocuments = ({ coreSchemaDef, filePathPatternMap, contentDirPath, contentTypeMap, flags, options, previousCache, verbose, }) => (OT.withSpan('@contentlayer/source-local/fetchData:fetchAllDocuments', { attributes: { contentDirPath } })(provideDocumentTypeMapState(T.gen(function* ($) {
    const allRelativeFilePaths = yield* $(getAllRelativeFilePaths({ contentDirPath }), fileName_1 + ":38:44");
    ...
    const { dataErrors, documents } = yield* $((T.map_(T.map_(T.forEachParN_(allRelativeFilePaths, concurrencyLimit, (relativeFilePath) => makeCacheItemFromFilePath({
        ...
        previousCache,
        contentTypeMap,
    }), fileName_1 + ":45:24"), Chunk.partitionThese, fileName_1 + ":56:16"), ({ tuple: [errors, docs] }) => ({ dataErrors: Chunk.toArray(errors), documents: Chunk.toArray(docs) }), fileName_1 + ":57:16")), fileName_1 + ":42:49");
    const singletonDataErrors = yield* $(validateSingletonDocuments({ coreSchemaDef, filePathPatternMap }), fileName_1 + ":61:43");
...
const getAllRelativeFilePaths = ({ contentDirPath, }) => {
    const filePathPattern = '**/*.{md,mdx,json,yaml,yml}';
    return (OT.withSpan('@contentlayer/source-local/fetchData:getAllRelativeFilePaths')(T.map_(T.tryCatchPromise(() => glob(filePathPattern, { cwd: contentDirPath }), (error) => new fs.UnknownFSError({ error }), fileName_1 + ":90:22"), (_) => _.map(posixFilePath), fileName_1 + ":94:10")));
};
const validateSingletonDocuments = ({ coreSchemaDef, filePathPatternMap, }) => T.gen(function* ($) {
    ...
    const documentTypeMap = yield* $(DocumentTypeMapState.get, fileName_1 + ":111:37");
