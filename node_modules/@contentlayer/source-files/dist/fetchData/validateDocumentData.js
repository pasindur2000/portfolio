...
const getDocumentDefNameByFilePathPattern = ({ relativeFilePath, filePathPatternMap, }) => {
    const matchingFilePathPattern = Object.keys(filePathPatternMap).find((filePathPattern) => minimatch(relativeFilePath, filePathPattern));
    if (matchingFilePathPattern) {
        return filePathPatternMap[matchingFilePathPattern];
    }
    return undefined;
};
const validateFieldData = ({ fieldDef, rawFieldData, documentFilePath, documentTypeName, contentDirPath, }) => T.orDie(T.gen(function* ($) {
    ...
    switch (fieldDef.type) {
        ...
        case 'reference':
            if (typeof rawFieldData === 'string') {
                ...
                const fileExists = yield* $(fs.fileOrDirExists(fullFilePath), fileName_1 + ":216:38");
                if (!fileExists) {
...
}, fileName_1 + ":195:8"), fileName_1 + ":232:19");
