...
export const makeCoreSchema = ({ documentTypeDefs, options, }) => T.gen(function* ($) {
    ...
    for (const documentDef of documentTypeDefs) {
        ...
        if (fieldDefs.some((_) => _.name === options.fieldOptions.bodyFieldName)) {
            yield* $(T.fail(new DuplicateBodyFieldError({ bodyFieldName: options.fieldOptions.bodyFieldName }), fileName_1 + ":29:24"), fileName_1 + ":29:17");
        }
        ...
        const computedFields = Object.entries(documentDef.computedFields ?? {}).map(([name, computedField]) => ({
            ...
            resolve: (_) => computedField.resolve(_),
        }));
    ...
    const hash = yield* $(utils.hashObject({ defs, options }), fileName_1 + ":93:26");
